...
class Processor {
    ...
    public:
        ...
        bool outputMatchesProgram(int instructionIdx) {
            runProgram();
...
{
    // A is processed in blocks of 3 bits for each output,
    // so we recursively check which 3 bits appended to A produce the next output, from end to beginning.
    for (uint64_t i = 0; i < 8; i++) {
        uint64_t nextA = currentA * 8 + i;
        if (Processor({nextA, 0, 0}, program).outputMatchesProgram(curInstruction)) {
            if (curInstruction == 0)
                return nextA;
